...
tags: [服务器,2FA,sso,单点登录,caddy,SSL证书]
category: [计算机技术]
lede: 轻量Web服务sso解决方案
...
在可预期的家宽限制条件下，我们假定Caddy在互联网出口IP的8443端口监听访问请求至对应的两个Homelab服务`sub1``sub2`，3A管理后台地址设为`auth.mydomain.com:8443`。由于涉及多个子域，Caddy为其统一申请通配符证书以简化配置。作为对照，`sub1`接入单点登录`sub2`保持裸连。  
```
{
    https_port 8443
...
```
完成配置后，请求`sub1.mydomain.com:8443`内容将被重定向至单点登录界面完成鉴权后才可访问而`sub2`不受此限制。  
